...
export default function MovieError({
  ...
  useEffect(() => {
    console.error('[Movie page error]', error.message);
  }, [error]);

  function handleRetry() {
    setClicked(true);
    startTransition(() => {
      router.refresh(); // bust Next.js router cache → force fresh server fetch
  ...
  return (
    <div style={{
      ...
      <p style={{ color: '#8b949e', maxWidth: '400px', marginBottom: '1.5rem' }}>
        There was a temporary issue fetching movie data. This usually fixes itself — try refreshing.
      </p>
