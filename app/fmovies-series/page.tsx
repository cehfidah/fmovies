...
export const metadata: Metadata = {
  title: 'Watch Free TV Series Online — Fmovies',
  description:
    'Stream free TV series and shows online in HD on Fmovies. Thousands of episodes available — no subscription needed.',
  alternates: { canonical: 'https://fmoviesz.cyou/fmovies-series' },
};

export const revalidate = 3600;

const SORT_OPTIONS = [
  { label: 'Latest Release', value: 'first_air_date.desc' },
...
export default async function SeriesPage({ searchParams }: Props) {
  const { page: pageStr, sort = 'first_air_date.desc', year = '' } = await searchParams;
  const page = Math.max(1, parseInt(pageStr || '1', 10));

  const params: Record<string, string> = {
    sort_by: SORT_OPTIONS.find(s => s.value === sort)?.value || 'first_air_date.desc',
    page: String(page),
    'vote_count.gte': '10',
  };
  if (year) params.first_air_date_year = year;

  const data = await discoverTV(params).catch(() => ({ results: [], total_pages: 1 }));
  const basePath = `/fmovies-series?sort=${sort}${year ? `&year=${year}` : ''}&`;
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem 1rem' }}>
        {/* Page title */}
        <h1 className="section-heading" style={{ fontSize: '1.4rem', marginBottom: '1.25rem' }}>Watch TV Series</h1>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            {SORT_OPTIONS.map(opt => (
              <a key={opt.value + opt.label}
                href={`/fmovies-series?sort=${opt.value}${year ? `&year=${year}` : ''}`}
                style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            <a href={`/fmovies-series?sort=${sort}`}
              style={{
            ...
            {YEARS.map(y => (
              <a key={y} href={`/fmovies-series?sort=${sort}&year=${y}`}
                style={{
                  padding: '4px 12px', borderRadius: '20px', fontSize: '0.8rem',
        ...
        <MovieGrid movies={(data.results || []).map((m: any) => ({ ...m, media_type: 'tv' }))} />
        <PaginationBar current={page} total={Math.min(data.total_pages || 1, 500)} basePath={basePath} />
      </main>
      <Footer />
    </>
  );
